...
class SaleOrder(models.Model):
    ...
    def action_invoice(self):
        ...
        created_invoice = self.create_inventory_output()
        self.write({'state': 'invoiced'})
        if not created_invoice:
            self.env.cr.commit()
            raise exceptions.Warning('El documento de inventario no ha sido creado debido a que no han establecido las ubicaciones para los tipos de factura. Contacte al administrador  ')
    ...
    def create_inventory_output(self, is_canceled=False):
        note = self.partner_id.display_name + '\n Factura Nro:' + self.nro_invoice
        name = self.name.replace('OV', 'FV')
        ...
        StockMove = self.env['guazu.stock.move']
        move_lines_vals = []
        for line in self.line_ids:
            move_lines_vals.append((0, False, {
                'product_id': line.product_id.id,
                'quantity': line.quantity,
                'uom_id': line.uom_id.id,
                'auto_price': False,  # Cambiar a True si querés que se calcule automático
                'price': line.product_id.price,
                'track_id': line.product_id.use_tracks,
            }))

        move_vals = {
            'name': name,
            'emission_date': datetime.now(), #fields.Date.context_today(record),
            'done_date': datetime.now(), #fields.Date.context_today(record),
            'note': note,
            'company_id': company,
            'currency_id': currency,
            'state': state,
            'location_id': location_o,
            'location_dest_id': location_d,
